...
const PALETTES = {
  viral: { a: '#c51f2b', b: '#3a0b0f', c: '#050505', text: '#ffffff', accent: '#d9a441' },
...
function hasVisualEffect(effectText, pattern) {
  return pattern.test(effectText);
}
...
export function wrapLines(ctx, text, maxWidth, maxLines = 5) {
...
export function wrapBidiLines(ctx, text, maxWidth, maxLines = 5, baseRTL = false) {
  const words = String(text || '').split(/\s+/).filter(Boolean);
  const lines = [];
  let current = [];
...
export function drawBidiLines(ctx, bidiLines, centerX, startY, lineHeight) {
...
export function drawStudioFrame(ctx, { project, timeline, elapsedSeconds, width, height, showWatermark = true, backgroundImage = null }) {
  const palette = paletteFor(project);
  const visualEffects = visualEffectsFor(project);
  const zoomEffect = hasVisualEffect(visualEffects, /zoom|ken burns|rapprochement/);
  const softEffect = hasVisualEffect(visualEffects, /mouvement doux|ralenti|slow motion|minimaliste|epure/);
  const fastEffect = hasVisualEffect(visualEffects, /transition(?:s)? rapide|jump cut|flash|energi|rythme rapide/);
  const motionSeconds = elapsedSeconds * (softEffect ? 0.65 : fastEffect ? 1.28 : 1);
  ...
  if (backgroundImage) {
    // ---- fond photo pro : cover-fit + Ken Burns par scène + voiles de lisibilité ----
    const imgW = backgroundImage.width || width;
    const imgH = backgroundImage.height || height;
    const zoomRange = zoomEffect ? 0.09 : softEffect ? 0.028 : 0.045;
    const zoom = 1.04 + (entry.index % 2 === 0 ? progress * zoomRange : (1 - progress) * zoomRange);
    const coverScale = Math.max(width / imgW, height / imgH) * zoom;
    const drawW = imgW * coverScale;
    const drawH = imgH * coverScale;
    ctx.drawImage(backgroundImage, (width - drawW) / 2, (height - drawH) / 2 - (elapsedSeconds % 1) * 0, drawW, drawH);

    const topScrim = ctx.createLinearGradient(0, 0, 0, height * 0.24);
    topScrim.addColorStop(0, 'rgba(7, 10, 18, 0.78)');
    ...
    bottomScrim.addColorStop(0.55, 'rgba(7, 10, 18, 0.66)');
    bottomScrim.addColorStop(1, 'rgba(7, 10, 18, 0.94)');
    ctx.fillStyle = bottomScrim;
    ctx.fillRect(0, height * 0.42, width, height * 0.58);

    const tint = ctx.createLinearGradient(0, 0, width, height);
    tint.addColorStop(0, hexToRgba(palette.a, 0.10));
    tint.addColorStop(1, hexToRgba(palette.b, 0.14));
    ctx.fillStyle = tint;
    ctx.fillRect(0, 0, width, height);
  } else {
    // ---- fond : dégradé animé + halo ----
    const drift = Math.sin((motionSeconds * 0.35) + entry.index) * 0.06;
    ...
    ctx.save();
    if (softEffect) ctx.globalAlpha = 0.58;
    drawBackdropMotif(ctx, { width, height, elapsedSeconds: motionSeconds, palette, sceneIndex: entry.index });
    ctx.restore();
  }

  // ---- traitements visuels demandés dans la phrase libre ----
  if (hasVisualEffect(visualEffects, /lumiere doree|reflets dores|golden|dore|ذهبي/)) {
    const gold = ctx.createLinearGradient(0, 0, width, height);
    gold.addColorStop(0, 'rgba(255, 224, 138, 0.17)');
    gold.addColorStop(0.5, 'rgba(255, 190, 11, 0.03)');
    gold.addColorStop(1, 'rgba(91, 52, 10, 0.14)');
    ctx.fillStyle = gold;
    ctx.fillRect(0, 0, width, height);
  }
  if (hasVisualEffect(visualEffects, /cinematique|cinema|سينمائي/)) {
    const cinema = ctx.createLinearGradient(0, 0, 0, height);
    cinema.addColorStop(0, 'rgba(0,0,0,0.28)');
    cinema.addColorStop(0.18, 'rgba(0,0,0,0)');
    cinema.addColorStop(0.82, 'rgba(0,0,0,0)');
    cinema.addColorStop(1, 'rgba(0,0,0,0.32)');
    ctx.fillStyle = cinema;
    ctx.fillRect(0, 0, width, height);
  }
  if (scene.role === 'proof' && hasVisualEffect(visualEffects, /avant \/ apres|avant apres|before after|قبل وبعد/)) {
    ctx.fillStyle = 'rgba(6, 10, 16, 0.28)';
    ctx.fillRect(0, 0, width / 2, height);
    ctx.fillStyle = palette.accent;
    ctx.fillRect(width / 2 - 2, height * 0.13, 4, height * 0.72);
  }

  // ---- Ken Burns sur le bloc contenu ----
  const kbRange = zoomEffect ? 0.085 : softEffect ? 0.032 : 0.05;
  const kb = entry.index % 2 === 0 ? 1.03 + progress * kbRange : 1.03 + kbRange - progress * kbRange;

  // ---- flash de coupe entre scènes ----
  const flashDuration = fastEffect ? 0.18 : 0.12;
  if (entry.index > 0 && local < flashDuration) {
    ctx.save();
    ctx.globalAlpha = (1 - local / flashDuration) * (fastEffect ? 0.68 : 0.5);
  ...
  const pop = easeOutBack(clamp01(local / (softEffect ? 0.62 : fastEffect ? 0.26 : 0.38)));
  ctx.save();
