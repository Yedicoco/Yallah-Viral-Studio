...
// Coordonnées et pages officielles Yallah Services — source de vérité unique.
// Utilisées comme valeurs par défaut du formulaire, du CTA, de la caption et de l'API.
export const YALLAH_CONTACT = {
  gsm: '+212 691733585',
  whatsapp: '+212 691733585',
  email: 'servicesyallah@gmail.com',
  tiktok: {
    handle: '@yallah.services.m',
    url: 'https://www.tiktok.com/@yallah.services.m'
  },
  instagram: {
    handle: '@yallahservice',
    url: 'https://www.instagram.com/yallahservice'
  }
};
...
function cleanEmail(value, fallback = YALLAH_CONTACT.email) {
  const candidate = cleanText(value, fallback);
  return /^[^\s@]+@[^\s@]+\.[^\s@]{2,}$/.test(candidate) ? candidate : fallback;
}

function cleanPhone(value, fallback = YALLAH_CONTACT.gsm) {
  const candidate = cleanText(value, fallback);
  const digits = candidate.replace(/\D/g, '');
  return /^\+?[\d ()-]+$/.test(candidate) && digits.length >= 9 && digits.length <= 15
    ? candidate
    : fallback;
}

// Accepte un handle (« @yallahservice ») ou une URL de profil, et retombe sur la page officielle.
function cleanHandle(value, fallback) {
  const raw = cleanText(value, fallback).replace(/^https?:\/\/(?:www\.)?(?:tiktok|instagram)\.com\//i, '');
  const handle = `@${raw.split(/[/?#]/)[0].replace(/^@+/, '')}`;
  return /^@[a-zA-Z0-9._]{2,32}$/.test(handle) ? handle : fallback;
}
...
function contactBlock(input = {}) {
  const tiktokHandle = cleanHandle(input.tiktok, YALLAH_CONTACT.tiktok.handle);
  const instagramHandle = cleanHandle(input.instagram, YALLAH_CONTACT.instagram.handle);

  return {
    gsm: cleanPhone(input.whatsapp, YALLAH_CONTACT.gsm),
    whatsapp: cleanPhone(input.whatsapp, YALLAH_CONTACT.gsm),
    email: cleanEmail(input.email, YALLAH_CONTACT.email),
    tiktok: {
      handle: tiktokHandle,
      url: socialUrl('tiktok', tiktokHandle)
    },
    instagram: {
      handle: instagramHandle,
      url: socialUrl('instagram', instagramHandle)
    }
  };
}

function ctaText(input, language) {
  const whatsapp = cleanPhone(input.whatsapp, YALLAH_CONTACT.gsm);
...
function contactLine(input, language) {
  const contact = contactBlock(input);

  if (language === 'darija') {
    return `📲 WhatsApp : ${contact.gsm} · ✉️ ${contact.email}`;
  }
  if (language === 'ar') {
    return `📲 واتساب: ${contact.gsm} · ✉️ ${contact.email}`;
  }
  return `📲 WhatsApp : ${contact.gsm} · ✉️ ${contact.email}`;
}

function socialLine(input, language) {
  const contact = contactBlock(input);

  if (language === 'darija') {
    return `Tbi3ouna : TikTok ${contact.tiktok.handle} · Instagram ${contact.instagram.handle}`;
  }
  if (language === 'ar') {
    return `تابعونا: تيك توك ${contact.tiktok.handle} · إنستغرام ${contact.instagram.handle}`;
  }
  return `Suivez-nous : TikTok ${contact.tiktok.handle} · Instagram ${contact.instagram.handle}`;
}

// CTA de suivi : transforme une vue en abonné, sans promesse de viralité.
function followCta(input, language) {
  const contact = contactBlock(input);

  if (language === 'darija') {
    return `Tbi3ou Yallah Services 3la TikTok (${contact.tiktok.handle}) w Instagram (${contact.instagram.handle}) bash matfawtouch 7ta chi nasiha.`;
  }
  if (language === 'ar') {
    return `تابعوا يلاه سيرفيسز على تيك توك (${contact.tiktok.handle}) وإنستغرام (${contact.instagram.handle}) لكي لا يفوتكم أي نصيحة.`;
  }
  return `Suivez Yallah Services sur TikTok (${contact.tiktok.handle}) et Instagram (${contact.instagram.handle}) pour ne manquer aucun conseil.`;
}
...
function buildSceneTemplates(input, service, styleData, hooks, viralBoost) {
  ...
  const gsm = contactBlock(input).gsm;
  ...
  const scenePool = [
    ...
    },
    {
      ...
      overlay: sentenceByLanguage(input.language,
        `WhatsApp ${gsm} · réponse rapide à ${city}.`,
        `WhatsApp ${gsm} · jawb sari3 f ${city}.`,
        `واتساب ${gsm} · رد سريع في ${city}.`),
      voice: cta,
      transition: 'zoom CTA',
      emoji: '📲'
    },
  ...
  return selected.map((scene, index) => ({
    ...
    prompt: `${scene.visual}. Format TikTok/Reels 9:16, ${styleData.energy}, lumière naturelle, aucun texte illisible, marque Yallah Services.${input.creativeDirection ? ` Direction créative demandée : ${input.creativeDirection}.` : ''} Objectif: ${objective}.`
  }));
}
...
function buildCaption(input, service, hooks) {
  const city = cleanText(input.city, 'Casablanca');
  const serviceLocalized = serviceName(service, input.language);
  const contact = contactLine(input, input.language);
  const socials = socialLine(input, input.language);
  ...
  return `${hooks[0]}\n\nBesoin de ${serviceLocalized} à ${city} ? Envoyez-nous votre besoin sur WhatsApp ou par email et l’équipe Yallah Services vous répond.\n\n${contact}\n${socials}`;
}
...
function buildOptimization(input, hooks, scenes, viralBoost, generated = {}) {
  const totalDuration = scenes.reduce((sum, scene) => sum + scene.duration, 0);
  const hookWords = hooks[0].split(/\s+/).length;
  const sceneCount = scenes.length;
  const contact = contactBlock(input);
  const contactZone = `${generated.cta || ''} ${generated.caption || ''}`;
  const hasOfficialContacts = [contact.gsm, contact.email, contact.tiktok.handle, contact.instagram.handle]
    .every(value => Boolean(value) && contactZone.includes(value));
  const checks = [
    {
    ...
    {
      label: 'Coordonnées et pages officielles Yallah Services',
      status: hasOfficialContacts ? 'ok' : 'warning',
      tip: hasOfficialContacts
        ? `GSM ${contact.gsm}, email ${contact.email}, TikTok ${contact.tiktok.handle} et Instagram ${contact.instagram.handle} présents dans le CTA et la caption.`
        : 'Renseigner le GSM/WhatsApp, l’email et les pages TikTok/Instagram officielles avant publication.'
    }
  ];
...
function normalizeInput(input = {}) {
  ...
  return {
    objective: cleanText(input.objective, 'Je veux trouver une femme de ménage à Casablanca', 500),
    city: cleanText(input.city, 'Casablanca', 100),
    whatsapp: cleanPhone(input.whatsapp, YALLAH_CONTACT.gsm),
    email: cleanEmail(input.email, YALLAH_CONTACT.email),
    tiktok: cleanHandle(input.tiktok, YALLAH_CONTACT.tiktok.handle),
    instagram: cleanHandle(input.instagram, YALLAH_CONTACT.instagram.handle),
    service,
    style,
    language,
    duration,
    audience: cleanText(input.audience, SERVICES[service].audienceFr, 200),
    creativeDirection: cleanText(input.creativeDirection, '', 500),
    effects: (Array.isArray(input.effects) ? input.effects : [])
      .map(effect => cleanText(effect, '', 100))
      .filter(Boolean)
      .slice(0, 6)
  };
}
...
// Fusionne la couche créative produite par un LLM (lib/llm.mjs) avec les
// garanties du studio : coordonnées officielles imposées dans la scène CTA,
// longueurs bornées, hashtags normalisés, complétion par les templates.
function mergeCreativeLayer({ creative, input, scenes, hooksTpl, captionTpl, hashtagsTpl }) {
  const gsm = contactBlock(input).gsm;
  const gsmDigits = gsm.replace(/\D/g, '');
  const cta = ctaText(input, input.language);

  const hooks = (Array.isArray(creative.hooks) ? creative.hooks : [])
    .map(hook => cleanText(hook, '', 220))
    .filter(Boolean)
    .slice(0, 5);
  while (hooks.length < 5) hooks.push(hooksTpl[hooks.length % hooksTpl.length]);

  const mergedScenes = scenes.map((scene, index) => {
    const candidate = creative.scenes?.[index];
    if (!candidate) return scene;
    const next = { ...scene };
    if (candidate.voice) next.voice = cleanText(candidate.voice, scene.voice, 500);
    if (candidate.onScreenText) next.onScreenText = cleanText(candidate.onScreenText, scene.onScreenText, 220);
    next.caption = next.voice.length > 118 ? `${next.voice.slice(0, 115)}…` : next.voice;
    return next;
  });

  // Garantie serveur : la scène CTA garde toujours les coordonnées officielles.
  const last = mergedScenes[mergedScenes.length - 1];
  if (last) {
    const templateLast = scenes[scenes.length - 1];
    if (!last.voice.replace(/\s/g, '').includes(gsmDigits)) {
      last.voice = `${last.voice.replace(/\s+$/, '')} ${cta}`.slice(0, 300).trim();
    }
    if (!last.onScreenText.replace(/\s/g, '').includes(gsmDigits)) {
      last.onScreenText = templateLast.onScreenText;
    }
    last.caption = last.voice.length > 118 ? `${last.voice.slice(0, 115)}…` : last.voice;
  }

  const sanitizeHashtag = tag => {
    const raw = `#${String(tag || '').trim().replace(/^#+/, '').replace(/\s+/g, '')}`;
  ...
  const hashtags = (creative.hashtags || [])
    ...
    .filter(tag => tag !== '#YallahServices');
  const finalHashtags = ['#YallahServices', ...hashtags];
  for (const tag of hashtagsTpl) {
    if (finalHashtags.length >= 12) break;
    if (!finalHashtags.includes(tag)) finalHashtags.push(tag);
  }

  return {
    hooks,
    scenes: mergedScenes,
    caption: creative.caption ? cleanText(creative.caption, '', 2_000) : captionTpl,
    hashtags: finalHashtags.slice(0, 12)
  };
}
...
export function generateStudioProject(rawInput = {}, options = {}) {
  ...
  let hooks = buildHooks(input, service, input.style, Boolean(options.viralBoost));
  let scenes = buildSceneTemplates(input, service, styleData, hooks, Boolean(options.viralBoost));
  let hashtags = buildHashtags(input, service, input.style);
  let caption = buildCaption(input, service, hooks);
  const cta = ctaText(input, input.language);

  if (options.creative) {
    const merged = mergeCreativeLayer({
      creative: options.creative,
      input,
      scenes,
      hooksTpl: hooks,
      captionTpl: caption,
      hashtagsTpl: hashtags
    });
    hooks = merged.hooks;
  ...
  const contact = contactBlock(input);
  const optimization = buildOptimization(input, hooks, scenes, Boolean(options.viralBoost), { cta, caption });
  ...
  return {
    ...
    subtitles,
    contact,
    contactLine: contactLine(input, input.language),
    socialLine: socialLine(input, input.language),
    followCta: followCta(input, input.language),
    cta,
    caption,
    hashtags,
    music: {
    ...
    textEngine: options.textEngine || {
      kind: 'templates',
      label: 'Moteur de templates déterministe',
      note: 'Brancher un LLM local pour des hooks originaux à chaque génération (voir docs/llm-local.md).'
    },
    viralBoost: Boolean(options.viralBoost)
  };
}

export function improveStudioProject(project = {}, options = {}) {
  const baseInput = project.input || project;
  const improvedInput = {
    ...baseInput,
    style: 'viral'
  };
  const improved = generateStudioProject(improvedInput, {
    viralBoost: true,
    creative: options.creative,
    textEngine: options.textEngine
  });
  ...
  improved.improvementNotes = [
    ...
    'CTA WhatsApp (GSM officiel Yallah Services) répété dans la voix off, l’écran final et la caption, avec l’email officiel en complément.',
    'Appel à suivre les pages TikTok et Instagram officielles pour convertir la vue en abonné.',
