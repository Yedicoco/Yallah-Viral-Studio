...
// Coordonnées officielles Yallah Services — source de vérité unique.
...
function buildOptimization(input, hooks, scenes, viralBoost, generated = {}) {
  ...
  const hasOfficialContacts = contactZone.includes(contact.gsm) && contactZone.includes(contact.email);
  const checks = [
    {
    ...
    {
      label: 'Coordonnées officielles Yallah Services',
      status: hasOfficialContacts ? 'ok' : 'warning',
      tip: hasOfficialContacts
        ? `GSM/WhatsApp ${contact.gsm} et email ${contact.email} présents dans le CTA et la caption.`
        : 'Renseigner le GSM/WhatsApp Yallah Services et l’email officiel avant publication.'
    }
  ];
