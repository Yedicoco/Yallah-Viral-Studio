...
function contactBlock(input = {}) {
  ...
  return {
    gsm: cleanText(input.whatsapp, YALLAH_CONTACT.gsm),
    whatsapp: cleanText(input.whatsapp, YALLAH_CONTACT.gsm),
...
function ctaText(input, language) {
  const whatsapp = cleanText(input.whatsapp, YALLAH_CONTACT.gsm);
...
function mergeCreativeLayer({ creative, input, scenes, hooksTpl, captionTpl, hashtagsTpl }) {
  ...
  const mergedScenes = scenes.map((scene, index) => {
    ...
    const next = { ...scene };
    if (candidate.voice) next.voice = candidate.voice;
    if (candidate.onScreenText) next.onScreenText = candidate.onScreenText;
  ...
  return {
    hooks,
    scenes: mergedScenes,
    caption: creative.caption || captionTpl,
