...
const cleanupTimer = setInterval(cleanupOldJobs, 15 * 60 * 1_000);
cleanupTimer.unref();
...
function snapshot(job) {
  return {
    ...
    video: job.status === 'done'
      ...
      : null,
    project: job.status === 'done' ? { render: job.project?.render || null } : null
  };
}

export function createVideoJob(project, { mode = 'scenario', voiceText = '', ownerId = 'anonymous' } = {}) {
...
export function getVideoJob(id, ownerId = 'anonymous') {
  const job = jobs.get(id);
  if (!job || job.ownerId !== String(ownerId || 'anonymous')) return null;
  return snapshot(job);
}

export function getVideoJobFilePath(id, kind, ownerId = 'anonymous') {
  const job = jobs.get(id);
  if (!job || job.ownerId !== String(ownerId || 'anonymous') || job.status !== 'done') return null;
...
function remainingRenderTime(job) {
  return Math.max(1, RENDER_TIMEOUT_MS - (Date.now() - job.startedAt));
}

function assertWithinRenderDeadline(job) {
  if (remainingRenderTime(job) <= 1) {
    throw new Error('Rendu trop long, abandonné après 10 minutes');
  }
}
...
async function runJob(job) {
  ...
  try {
    ...
    for (let index = 0; index < scenes.length; index += 1) {
      assertWithinRenderDeadline(job);
    ...
    if (totalDuration > 120) {
      throw new Error('La voix dépasse 2 minutes après calage. Raccourcissez le scénario.');
    }
    ...
    }, { durationSeconds: totalDuration, timeoutMs: Math.min(8 * 60 * 1_000, remainingRenderTime(job)) });
...
async function runPosterJob(job) {
  ...
  // 2. durée : voix + respiration, bornée sans tronquer le message.
  if (clip.durationSeconds > 88.6) {
    throw new Error('La voix d’affiche dépasse 90 secondes. Raccourcissez le texte.');
  }
  const totalDuration = Math.max(8, clip.durationSeconds + 1.4);
  ...
  for (let frame = 0; frame < totalFrames; frame += 1) {
    ...
    await writeFile(join(framesDir, `frame_${String(frame).padStart(5, '0')}.jpg`), await canvas.encode('jpeg', 88));
  ...
  await runFfmpeg([
    ...
    durationSeconds: totalDuration,
    timeoutMs: Math.min(8 * 60 * 1_000, remainingRenderTime(job))
  });
...
function runFfmpeg(args, onPercent, { durationSeconds = 0, timeoutMs = 8 * 60 * 1_000 } = {}) {
  return new Promise((resolveEncoding, reject) => {
    ...
    child.stderr.on('data', chunk => {
      ...
      if (matches && onPercent && durationSeconds > 0) {
        const last = matches[matches.length - 1];
        const [, h, m, s] = last.match(/time=(\d+):(\d+):(\d+(?:\.\d+)?)/);
        const seconds = Number(h) * 3600 + Number(m) * 60 + Number(s);
        onPercent(Math.min(99, Math.round((seconds / durationSeconds) * 100)));
      }
    });
    const timer = setTimeout(() => {
      child.kill('SIGKILL');
      finish(reject)(new Error('Encodage FFmpeg trop long'));
    }, timeoutMs);
    child.on('error', finish(reject));
    child.on('close', code => {
      if (settled) return;
      if (code === 0) finish(resolveEncoding)();
      else finish(reject)(new Error(`FFmpeg a échoué (${code}) : ${stderr.slice(-400)}`));
    });
  });
}
