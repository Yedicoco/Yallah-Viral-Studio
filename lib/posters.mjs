...
function drawBrand(ctx, width, y, accent) {
  ctx.save();
  ...
  ctx.shadowBlur = 10;
  ctx.fillText('YALLAH SERVICES', width / 2, y);
  ctx.shadowBlur = 0;
  roundRectPath(ctx, width / 2 - 90, y + 18, 180, 7, 3.5);
...
export async function renderPoster(project, { format = 'story' } = {}) {
  ...
  const accent = PALETTE_BY_STYLE[project?.style?.code] || '#ffbe0b';
  ...
  drawBrand(ctx, width, brandY, accent);
  ...
  const headTop = isStory ? headY : headY + 120;
  drawBidiLines(ctx, headLines, width / 2, headTop, headLineHeight);
  ctx.restore();

  // 5. arguments (3 max) — coche dessinée + texte, ligne par ligne
  const benefits = benefitsFrom(project, isStory ? 3 : 2);
  const benefitsLeft = 130;
  const benefitsWidth = width - benefitsLeft * 2;
  let cursorY = headTop + headBlock + (isStory ? 70 : 44);
  ctx.save();
  ctx.font = `600 ${isStory ? 42 : 38}px ${FONT}`;
  ctx.textBaseline = 'middle';
  const lineH = isStory ? 52 : 48;
  ...
  const cardY = isStory ? height - 430 : height - 300;
  ...
  ctx.fillText('Yallah Services — mise en relation de confiance', width / 2, isStory ? height - 92 : height - 40);
  ctx.restore();
